...
function getExternalSchema() {
    $server_proto = "http";
    if ( !empty($_SERVER['HTTPS']) ) { $server_proto .= "s"; }
    if ( !empty($_SERVER['HTTP_X_FORWARDED_PROTO']) ) { $server_proto = $_SERVER['HTTP_X_FORWARDED_PROTO']; }
    return $server_proto;
}

function getExternalPort() {
    $server_port = $_SERVER['SERVER_PORT'];
    if ( !empty($_SERVER['HTTP_X_FORWARDED_PORT']) ) { $server_port = $_SERVER['HTTP_X_FORWARDED_PORT']; }
    return $server_port;
}

#==============================================================================
# EOF: External URL functions
#==============================================================================
...
if ( $result === "" ) {
    ...
    if ( empty($reset_url) ) {
        ...
        $method = getExternalSchema();
        $server_name = getExternalHost();
        $server_port = getExternalPort();
